...
export class OrderHeaderService {
  ...
  constructor(
    ...
    this.endpoint = `${UrlApiUtil.getApiUrl()}OrderHeader`;
  }

  getOrders(userName) {
    return this.http.get<OrderHeader[]>(`${this.endpoint}/GetOrders?userName=${userName}`);
  }

  getOrderById(orderId) {
    return this.http.get<OrderHeader[]>(`${this.endpoint}/GetOrderById?orderId=${orderId}`);
  }

  getOrdersByCustomerId(customerId, currencyId, startDate, endDate, saleSupportHeaderId) {
    return this.http.get<OrderHeader[]>(`${this.endpoint}/getOrdersByCustomerId?customerId=${customerId}`
      + `&currencyId=${currencyId}&startDate=${startDate}&endDate=${endDate}&saleSupportHeaderId=${saleSupportHeaderId}`);
  }

  saveOrder(form: FormData) {
    return this.http.post<OrderHeader>(`${this.endpoint}/SaveOrder`, form);
  }

  updateOrder(form: FormData) {
    return this.http.put<OrderHeader[]>(`${this.endpoint}/UpdateOrder`, form);
  }

  deleteOrder(orderId, deleteBy) {
    return this.http.delete<OrderHeader>(`${this.endpoint}/DeleteOrder?orderId=${orderId}&deleteBy=${deleteBy}`);
  }
