...
import { AuthorizationStatus, PurchaseOrderType } from '@app/shared/enums';
...
export class SalesSupportHeaderService {
  ...
  getSaleSupports(userName): Observable<any> {
    return this.http.get<SaleSupportHeader[]>(`${this.endpoint}/getSaleSupports?userName=${userName}`);
  }

  getSaleSupportById(saleSupportId): Observable<any> {
    return this.http.get<SaleSupportHeader[]>(`${this.endpoint}/getSaleSupportById?saleSupportId=${saleSupportId}`);
  }

  saveSaleSupport(form: FormData): Observable<any> {
    return this.http.post<SaleSupportHeader>(`${this.endpoint}/saveSaleSupport`, form);
  }

  updateSaleSupport(form: FormData): Observable<any> {
    return this.http.put<SaleSupportHeader[]>(`${this.endpoint}/updateSaleSupport`, form);
  }

  deleteSaleSupport(saleSupportId, deleteBy): Observable<any> {
    return this.http.delete<SaleSupportHeader>(`${this.endpoint}/deleteSaleSupport?saleSupportId=${saleSupportId}&deleteBy=${deleteBy}`);
  }

  getSaleSupportReport(form: FormData): Observable<any> {
    return this.http.post<Blob>(`${this.endpoint}/getSaleSupportReport`, form, { responseType: 'blob' as 'json' });
  }
  ...
  private getWaterMark(dataParent): any {
    if (dataParent.statusId === SalesReportStatus.Cancelled) {
      return {
  ...
  private getPeriodSection(dataParent, period, purchaseOrderType): any {
  ...
  private getDetailObject(cols: any, data: any, dataParent: any): any {
  ...
  private getTotalObject(cols: any, data: any, dataParent: any, taxes: any): any {
    let subTotalGral = 0;
    let totalGral = 0;
  ...
  getHistoryAuthorization(dataParent, listAuthorization, listAuthorizers, logoKR) {
    ...
    let bodyImages: any = [];
    // statusId
    if (dataParent.statusId !== SalesReportStatus.Rejected && dataParent.statusId !== SalesReportStatus.Created) {
  ...
  */
  ...
  getSaleSupportModels(customerId, startDate, endDate, currencyId): Observable<any> {
    return this.http.get<any[]>(`${this.endpoint}/getSaleSupportModels?customerId=${customerId}`
